...
// ═══════════════════════════════════════════════════
//  NODOS — tamaño adaptado a pantalla
// ═══════════════════════════════════════════════════
  ...
  { nodes, edges },
  {
    layout: {
      hierarchical: {
        enabled: true,
        direction: "UD",
        levelSeparation: isSmall ? 130 : 180,
        nodeSpacing:     isSmall ? 110 : 160,
        treeSpacing:     isSmall ? 140 : 200,
        sortMethod: "directed"
      }
    },
    physics: false,
    interaction: {
      ...
      hover: !isTouchDevice(),   // disable hover on touch
      tooltipDelay: 9999,
      navigationButtons: false,
      zoomView: true,
      dragView: true,
      // Better touch pinch zoom
      zoomSpeed: 0.5
    },
    nodes: { chosen: false },
    edges: { chosen: false }
...
function setEdgeMode(mode) {
  ...
  document.getElementById(map[mode]).className = "btn active";
  actualizar();
}
...
setInterval(() => {
  pulseT += 0.12;
  const t = (Math.sin(pulseT) + 1) / 2;

  pulseIDs.forEach(id => {
    const m = materiaMap[id];
    if (!m) return;
    const isCursable = esCursable(m);
    const isRendible = esRendible(m);
    if (!isCursable && !isRendible) return;

    const col = isCursable ? "22,211,238" : "16,185,129";
    const glow = 4 + t * 12;
...
network.on("click", p => {
  ...
  // On touch: show tooltip after state change
  if (isTouchDevice()) {
    showNodeTooltip(id);
  }
});
...
// ═══════════════════════════════════════════════════
//  LONG PRESS (touch) → show tooltip
// ═══════════════════════════════════════════════════

let longPressTimer = null;
let longPressFired = false;

document.getElementById("mynetwork").addEventListener("touchstart", e => {
  longPressFired = false;
  longPressTimer = setTimeout(() => {
    longPressFired = true;
    // Get node at touch position
    const touch = e.touches[0];
    const canvasPos = network.DOMtoCanvas({ x: touch.clientX, y: touch.clientY });
    const nodeId = network.getNodeAt({ x: touch.clientX, y: touch.clientY });
    if (nodeId !== undefined) {
...
document.getElementById("mynetwork").addEventListener("touchend", () => {
  clearTimeout(longPressTimer);
}, { passive: true });

document.getElementById("mynetwork").addEventListener("touchmove", () => {
  clearTimeout(longPressTimer);
}, { passive: true });
...
function showNodeTooltip(nodeId, evt) {
  const m = materiaMap[nodeId];
  if (!m) return;
  ...
  let hint = "";
  if (est === "pendiente") {
    if (esCursable(m)) hint = `<b>✓ Disponible para cursar</b> — tap para marcar como Regular`;
    else {
    ...
    hint = `<b>✓ Lista para rendir</b> — tap para marcar como Aprobada`;
  } else {
    hint = `Tap para resetear a Pendiente`;
  }
...
network.on("hoverNode", p => {
  ...
  showNodeTooltip(p.node, p.event);
});

network.on("blurNode", () => {
  if (!isTouchDevice()) tooltip.classList.remove("show");
});
...
// Close tooltip when tapping outside (on touch)
document.addEventListener("touchstart", e => {
  if (tooltip.classList.contains("show")) {
    if (!tooltip.contains(e.target)) {
      // Only close if not tapping the network (that will trigger its own flow)
      const netWrap = document.getElementById("mynetwork");
...
function actualizar() {
  ...
  materias.forEach(m => {
    ...
    nodes.update({
      ...
      font: { color: fontColor, size: isSmall ? 11 : 13, face: "IBM Plex Mono" },
      borderWidth: bw,
