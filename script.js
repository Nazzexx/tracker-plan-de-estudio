...
// ═══════════════════════════════════════════════════
//  ESTADO (Supabase — sincronizado en la nube)
// ═══════════════════════════════════════════════════
...
async function cargarDesdeNube() {
  ...
  const { data } = await sb
    .from("progreso")
    .select("estados, electivas")
    .eq("user_id", user.id)
    .maybeSingle();

  if (data) {
    estados = data.estados || {};
    estadosElectivas = data.electivas || {};
  }

  // Una vez cargado, actualizar la visual
  actualizar();
...
// Estado de electivas (se guarda junto con estados en Supabase)
const KEY_ELECTIVAS = "utn-sistemas-electivas"; // se mantiene por compatibilidad
let estadosElectivas = {};

function guardarElectivas() {
  guardar(); // reutiliza la misma función que guarda todo en Supabase
}
...
// El progreso de electivas se carga desde Supabase via cargarDesdeNube()
// No es necesario inicializar desde localStorage
